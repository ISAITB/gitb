...
//    UNUSED_RESOURCE(                                    "TDL-021", "Resource [%s] is included in the test suite archive but is never used.", WARNING),
    ...
    INVALID_SCRIPTLET_REFERENCE(                        "TDL-037", "Test case [%s] references a scriptlet [%s] that could not be located within the test case nor as a separate file in the test suite.", ERROR),
    ...
    POTENTIALLY_INVALID_ORGANISATION_VARIABLE(          "TDL-063", "Test cases refer to a custom organisation property [%s]. Ensure this is defined at runtime.", INFO),
    POTENTIALLY_INVALID_SYSTEM_VARIABLE(                "TDL-064", "Test cases refer to a custom system property [%s]. Ensure this is defined at runtime.", INFO),
    ...
    SCRIPTLET_REFERENCE_DID_NOT_MATCH_SCRIPTLET(        "TDL-083", "Test case [%s] defines a call step referring to a scriptlet file within the test suite, but the referred file [%s] is not a scriptlet.", ERROR),
    UNEXPECTED_SCRIPTLET_INPUT(                         "TDL-084", "Test case [%s] defines a call step for scriptlet [%s] that is providing an unexpected input [%s].", ERROR),
    MISSING_SCRIPTLET_INPUT(                            "TDL-085", "Test case [%s] defines a call step for scriptlet [%s] that does not provide an expected input [%s].", ERROR),
    DUPLICATE_OUTPUT(                                   "TDL-086", "Test case [%s] defines a %s step that requests the same output [%s] multiple times.", WARNING),
    UNEXPECTED_SCRIPTLET_OUTPUT(                        "TDL-087", "Test case [%s] defines a call step for scriptlet [%s] that requests an unsupported output [%s].", WARNING),
    EXTERNAL_SCRIPTLET_USED(                            "TDL-088", "Test cases call scriptlet [%s] from test suite [%s]. Ensure this will be available at runtime.", INFO),
    EXTERNAL_STATIC_IMPORT_USED(                        "TDL-089", "Test cases import resource [%s] from test suite [%s]. Ensure this will be available at runtime.", INFO),
    EXTERNAL_DYNAMIC_IMPORT_USED(                       "TDL-090", "Test cases import resources from test suite [%s] using expression [%s]. Ensure these will be available at runtime.", INFO),
    DUPLICATE_INPUT(                                    "TDL-091", "Test case [%s] defines a %s step with a duplicate input [%s].", ERROR),
    DUPLICATE_CONFIG(                                   "TDL-092", "Test case [%s] defines a %s step with a duplicate configuration value [%s].", ERROR)
    ;
