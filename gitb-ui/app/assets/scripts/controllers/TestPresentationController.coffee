class TestPresentationController
	...
	iconClassForStatus:(status) ->
		switch status
			...
			when @Constants.TEST_STATUS.WARNING then "fa fa-exclamation-triangle fa-fw fa-lg"
	...
	tooltipForStatus:(status) ->
		switch status
			...
			when @Constants.TEST_STATUS.WARNING then "Warning"
	...
	divClassForStatus:(status) =>
		switch status
			...
			when @Constants.TEST_STATUS.WARNING then "row execution alert-warning"
	...
	labelClassForStatus:(status) ->
		switch status
			...
			when @Constants.TEST_STATUS.WARNING then "label label-warning font11"
