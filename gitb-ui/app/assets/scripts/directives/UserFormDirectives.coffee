  ...
  ()->
    ...
    template: ''+
      '<form class="form-horizontal">'+
        ...
        '</div>'+
        '<div class="form-group" ng-if="!tbAdmin && !tbNew">'+
          '<label ng-class="{\'col-sm-3\': sso, \'col-sm-4\': !sso}" class="control-label" for="organization"><span ng-if="!sso">* </span>Organisation:</label>'+
      ...
      '<div>'+
        ...
        '<div ng-if="selfRegOptions.length > 0">'+
          '<div ng-class="{\'link-account-separator\': sso, \'bottom-margin\': !sso}">'+
              '<div class="bg-info div-rounded div-padded">'+
                  '<p>Select the community you want to register for and provide the details of your organisation. If your community is not listed you need to request from an administrator to register your organisation on your behalf.</p>'+
                  '<span ng-if="sso"><b>Privacy note:</b> By registering you grant your consent to link your name, email and EU Login user ID to your new organisation\'s administrator account.</span>'+
                  '<span ng-if="!sso"><b>Privacy note:</b> By registering you grant your consent to link the provided information to your new organisation\'s administrator account.</span>'+
              '</div>'+
          '</div>'+
          '<div class="form-group">'+
              '<label class="col-xs-3 control-label" for="community">* Community:</label>'+
              '<div class="col-xs-7">'+
                  '<select ng-if="selfRegOptions.length > 1" id="community" class="form-control" ng-model="model.selfRegOption" ng-options="option as option.communityName for option in selfRegOptions"><option value=""></option></select>'+
          ...
          '<div ng-class="{\'form-separator-popup\': sso, \'form-separator\': !sso}">'+
              '<h4 class="title">Organisation details <span uib-tooltip="Your organisation represents the member of the selected community through which will you be testing for conformance"><i class="fa fa-question-circle"></i></span></h4>'+
          '</div>'+
          '<div class="form-group">'+
              ...
              '</div>'+
              '<div class="form-control-static"><span uib-tooltip="A short form of your organisation\'s name to appear in reports"><i class="fa fa-question-circle"></i></span></div>'+
          '</div>'+
          '<div class="form-group">'+
              ...
              '</div>'+
              '<div class="form-control-static"><span uib-tooltip="The complete name of your organisation"><i class="fa fa-question-circle"></i></span></div>'+
          '</div>'+
          '<div class="form-group" ng-if="model.selfRegOption.templates.length > 0">'+
              ...
              '</div>'+
              '<div class="form-control-static"><span uib-tooltip="Predefined test configuration(s) for your organisation curated by the community administrator"><i class="fa fa-question-circle"></i></span></div>'+
          '</div>'+
          '<div ng-if="!sso">'+
            '<div class="form-separator">'+
                '<h4 class="title">Administrator account details <span uib-tooltip="Through your administrator account you can manage your organisation\'s users, configure your conformance statements and execute tests"><i class="fa fa-question-circle"></i></span></h4>'+
            '</div>'+
            '<div class="form-group">'+
