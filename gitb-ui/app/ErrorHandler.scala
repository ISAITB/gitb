...
class ErrorHandler @Inject() (legalNoticeManager: LegalNoticeManager)
  ...
  override def onServerError(request: RequestHeader, ex: Throwable): Future[Result] = {
    ...
    val errorAsJson = requestedWithHeader.isDefined || acceptHeader.exists(x => x.toLowerCase.contains("application/json")) || request.path.startsWith(Configurations.API_ROOT)
