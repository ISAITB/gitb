  ...
  def getUnlinkedUserAccountsForEmail(email: String): List[UserAccount] = {
    val results = exec(
      ...
      .map(x => new UserAccount(
        ...
        Communities(x._9, x._10, x._11, None, -1, None, None, selfregNotification = false, None, SelfRegistrationRestriction.NoRestriction.id.toShort, selfRegForceTemplateSelection = false, selfRegForceRequiredProperties = false, None)
      ))
    results.sorted
  }
