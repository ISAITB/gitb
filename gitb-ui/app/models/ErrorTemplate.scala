...
class ErrorTemplate (_id:Long, _name:String, _description:Option[String], _content:String, _default:Boolean, _community:Long)
{
  var id:Long = _id
  ...
  def this(_case:ErrorTemplates) =
    this(_case.id, _case.name, _case.description, _case.content, _case.default, _case.community)

  def toCaseObject:ErrorTemplates = {
    ErrorTemplates(id, name, description, content, default, community)
  }
