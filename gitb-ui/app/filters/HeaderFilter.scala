  ...
  def apply(next: (RequestHeader) => Future[Result])
    ...
    next(requestHeader).map { result =>
      result.withHeaders(
        ...
        "Content-Security-Policy" -> "default-src 'none'; font-src 'self'; script-src 'self'; connect-src 'self'; style-src 'unsafe-inline' 'self'; img-src 'self' data:; base-uri 'self'; frame-ancestors 'none';"
      )
    }
  }
