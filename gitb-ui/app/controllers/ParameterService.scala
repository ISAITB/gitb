...
class ParameterService @Inject() (authorizedAction: AuthorizedAction, cc: ControllerComponents, parameterManager: ParameterManager, authorizationManager: AuthorizationManager, communityLabelManager: CommunityLabelManager) extends AbstractController(cc) {
  ...
  def updateParameter(parameterId: Long) = authorizedAction { request =>
    ...
    if (parameterManager.checkParameterExistsForEndpoint(parameter.name, parameter.endpoint, Some(parameterId))) {
      ResponseConstructor.constructBadRequestResponse(500, "A parameter with this name already exists for the " + communityLabelManager.getLabel(request, models.Enums.LabelType.Endpoint, single = true, lowercase = true)+".")
    } else{
      parameterManager.updateParameterWrapper(parameterId, parameter.name, parameter.desc, parameter.use, parameter.kind, parameter.adminOnly, parameter.notForTests, parameter.hidden, parameter.allowedValues, parameter.dependsOn, parameter.dependsOnValue)
      ResponseConstructor.constructEmptyResponse
    }
  }
