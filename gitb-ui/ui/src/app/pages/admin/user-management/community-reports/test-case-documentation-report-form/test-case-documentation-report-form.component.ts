...
export class TestCaseDocumentationReportFormComponent extends SimpleDocumentationReportFormComponent {
  ...
  reportType(): number {
    return this.Constants.REPORT_TYPE.TEST_CASE_DOCUMENTATION_REPORT
  }

  protected previewFallbackFileName(): string {
    return 'test_case_documentation.pdf'
  }

  protected previewCall(reportSettings: CommunityReportSettings): Observable<HttpResponse<ArrayBuffer>> {
    return this.reportService.exportDemoTestCaseDocumentationReport(this.communityId, reportSettings)
  }
