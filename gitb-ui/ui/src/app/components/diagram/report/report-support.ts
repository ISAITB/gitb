...
export abstract class ReportSupport extends BaseComponent {
  ...
  private openEditorWindow(name: string, value: ValueWithMetadata, assertionReports?: AssertionReport[], lineNumber?: number) {
    if (value != undefined && value.value != undefined && value.valueType != ValueType.string && value.mimeType != undefined && this.dataService.isImageType(value.mimeType)) {
      ...
      const modal = this.modalService.open(CodeEditorModalComponent, { modalDialogClass: 'modal-lg' })
